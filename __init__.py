...
"""hermes-memory-zh — Chinese semantic memory provider using MemoryProvider interface.
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "hermes-memory-zh", default={}) or {}
    except Exception:
        return {}
...
class HermesMemoryZhProvider(MemoryProvider):
    ...
    @property
    def name(self) -> str:
        return "hermes-memory-zh"

    def is_available(self) -> bool:
        return True  # SQLite is always available, numpy is optional

    def save_config(self, values, hermes_home):
        """Write config to config.yaml under plugins.hermes-memory-zh."""
        ...
        try:
            ...
            existing["plugins"]["hermes-memory-zh"] = values
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Semantic embedding — OpenAI-compatible gateway (e.g. DashScope compatible-mode or your own AI gateway).
        ...
        if embedding_enabled and embedding_api_key:
...
def register(ctx) -> None:
    """Register the hermes-memory-zh memory provider with the plugin system."""
    config = _load_plugin_config()
    provider = HermesMemoryZhProvider(config=config)
